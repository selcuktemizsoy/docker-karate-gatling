...
class UserSimulation extends Simulation{
  ...
  setUp(
    getSingleUser.inject(constantUsersPerSec(1000) during(300 seconds)).protocols(protocol),
    postSingleUser.inject(constantUsersPerSec(1000) during(300 seconds)).protocols(protocol)
  )

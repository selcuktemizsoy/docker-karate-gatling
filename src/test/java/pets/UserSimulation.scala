...
class UserSimulation extends Simulation{
  ...
  setUp(
    getSingleUser.inject(constantUsersPerSec(userCount.toInt) during(10 minutes)).protocols(protocol),
    postSingleUser.inject(constantUsersPerSec(userCount.toInt) during(10 minutes)).protocols(protocol)
  )
